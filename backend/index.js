...
import { Order, Food, Tag, Ingredient, Drink, Address, Offer, Side, FoodAndSides, FoodAndDrinks, OrderIndividual, User } from "./schema.js";
...
app.get("/tags", async (req, res) => {
  try {
    ...
    tags.forEach(tag => { tag.image = `${baseURL}/${tag.image}` })
    res.json(tags);
  } catch (error) {
...
app.get("/offers", async (req, res) => { //this need revision
  try {
    let foods = await Food.find({})
      .populate("tags")
      .populate("ingredients")
...
app.get("/sides/:id", async (req, res) => {
  try {
    const id = req.params.id
    console.log(id)
    const foodAndSides = await FoodAndSides.findOne({ food: id }).populate('sides')
    if (foodAndSides)
      res.json(foodAndSides.sides);
...
app.get("/drinks/:id", async (req, res) => {
  try {
    const id = req.params.id
    const foodAndDrinks = await FoodAndDrinks.findOne({ food: id }).populate('drinks')
    if (foodAndDrinks)
      res.json(foodAndDrinks.drinks);
  } catch (error) {
    console.error("Error fetching drinks:", error);
    res.status(500).json({ error: "Internal server error" });
  }
